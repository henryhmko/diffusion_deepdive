...
class DiffusionModel:
    ...
    def sample(self, model, n, diffusion_model, img_size, device, run_name):
        logging.info(f"Sampling {n} new images...")
        model.eval()
        with torch.no_grad():
            #generate results with each diffusion step
            for img_num in tqdm(range(2), desc="Generating Diffusion Step Samples..."): #generate 2 plots of diffusion process
                ...
                for i in reversed(range(diffusion_model.timesteps)):
                    ...
                    img = diffusion_model.reverse_process(img, t, model)
                    if i % 50 == 0:
                        ax[idx].imshow(reverse_transform(img[0]))
                        idx += 1

                new_path = os.path.join("results", run_name, f"result_img_{img_num}.png")
                plt.tight_layout()
